...
桌面上快捷方式太多、又舍不得删的话，可以拿这个把其中几个收进一个 2×2 的小格子里。
每个格子只占一个图标的位置，里面放四个程序，双击就启动，不弹面板。
...
这是第一个公开版本，请当成「能用，但还在打磨」。

- 功能是完整的。四宫格、管理器、参数设置、开机自启都能正常跑
...
- `config.json` 管全局，`group-*.json` 管每个格子放什么
- 以 `_` 开头的字段是注释，程序会忽略。所以可以直接把配置文件发给别人或 AI 帮忙改
- 管理器保存时是就地替换数值，注释和排版会保留（有回归测试守着）

### 图标缩小后可能认不出来

格子只有 37×48 像素，图标会被缩到 24×24，只有原尺寸的一小部分。

设计简洁的图标没事。细节多、文字多、靠颜色区分的，缩完容易糊成一团，几个格子看着都差不多。
标签也会被截断，7pt 字号下最多三个汉字。

如果缩完认不出，建议自己重绘一版小尺寸专用的简化图标，纯色块加一个大字母就够用。
重绘好之后在管理器里把「图标」那一栏填成你的 `.ico` / `.png` 路径，原程序不用动。

不想重绘的话，也可以把「图标边长」调大、「文字区高度」调小，或者干脆换成四个不容易混淆的程序。
...
那 79 MB 不是程序自己吃的，是 .NET 桌面运行时的基线，一个空的 WinForms 窗口也要占几十 MB。
对内存敏感的话建议自己实测再决定。
